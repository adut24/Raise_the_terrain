...
int poll_events(SDL_Instance instance, int *angle, SDL_Point grid[8][8],
				SDL_Point copy_grid[8][8], int alt[8][8], char *file)
{
	SDL_Event event;
		...
		switch (event.type)
		{
			...
			case SDL_KEYDOWN:
						...
						|| key.keysym.scancode == SDL_SCANCODE_RIGHT)
				{
					...
					init_iso_grid(copy_grid, alt, file);
...
int main(int ac, char **av)
{
	...
	init_2d_grid(grid);
	grid_copy(grid, copy_grid);
	init_iso_grid(copy_grid, alt, av[1]);
	...
	while (1)
	{
		if (poll_events(instance, &angle, grid, copy_grid, alt, av[1])
			== 1)
			break;
